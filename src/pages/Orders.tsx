                ...
                paginatedOrders.map((order: Order) => (
                  <tr key={order._id} className="group hover:bg-gray-50/30 transition-colors">
                    ...
                    <td className="px-6 py-5">
                      ...
                      {order.couponCode && (
                        <div className="mt-1 flex items-center gap-1 text-[10px] font-extrabold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-md w-fit">
                          <Ticket className="w-3 h-3 text-emerald-600" />
                          <span>{order.couponCode} (-₹{order.discountAmount || 0})</span>
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-5">
              ...
              <div className="p-5 rounded-2xl bg-gradient-to-br from-green-50/50 to-green-50/20 border border-green-100">
                ...
                <div className="space-y-3">
                  ...
                  {selectedOrder.couponCode && (
                    <div className="flex items-center justify-between bg-emerald-50/90 p-3 rounded-xl border border-emerald-200">
                      <div className="flex items-center gap-2">
                        <Ticket className="w-4 h-4 text-emerald-600" />
                        <div>
                          <p className="text-xs font-black text-emerald-900">Coupon Used ({selectedOrder.couponCode})</p>
                          <p className="text-[10px] font-semibold text-emerald-700">Customer saved ₹{selectedOrder.discountAmount || 0}</p>
                        </div>
                      </div>
                      <Badge className="bg-emerald-600 text-white font-extrabold text-xs">
                        -₹{selectedOrder.discountAmount || 0} OFF
                      </Badge>
                    </div>
                  )}
                  <div className="flex items-center justify-between">
